...
static struct s3c2410_platform_i2c ft5x0x_i2c_data __initdata = {
	.flags		= 0,
	.bus_num	= 2,
	.slave_addr	= 0x10,
	.frequency	= 250*1000,
...
//#define MULTITOUCH_RESET_GPIO 170
//#define MULTITOUCH_INT_GPIO 157  //You should modify it here

#define GTP_RST_PORT    S5PV210_GPH1(6)
//#define GTP_INT_PORT    S5PV210_GPH1(7)
#define GTP_INT_PORT    S5PV210_GPH1(2)
...
static void sitronix_reset_ic(void){
	printk("%s\n", __FUNCTION__);
	gpio_request(GTP_RST_PORT, "Multitouch Reset");
...
{
	//printk("%s\n", __FUNCTION__);
	//return gpio_get_value(MULTITOUCH_INT_GPIO);
        return gpio_get_value(GTP_INT_PORT);
}
#endif
...
static struct i2c_board_info mini210_i2c_devs2[] __initdata = {
...
#ifdef CONFIG_TOUCHSCREEN_FT5X0X
	{
		...
		//I2C_BOARD_INFO(SITRONIX_I2C_TOUCH_DRV_NAME, 0x54),
		.irq			= IRQ_EINT10,
...
#ifdef CONFIG_TOUCHSCREEN_FT5X0X
	s3c_i2c2_set_platdata(&ft5x0x_i2c_data);
#else
